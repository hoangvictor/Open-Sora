...
VIDEO_GENERATION_PROMPTS = [d['prompt_en'] for d in json.load(open('/Open-Sora/data/VBench_full_info.json'))]
N_PROMPTS = len(VIDEO_GENERATION_PROMPTS)
...
assert len(QUANT_MODES) == 1
assert len(VIDEO_RESOLUTIONS) == 1
assert len(VIDEO_LENGTHS) == 1
...
def main():
    ...
    # Save dir
    now = datetime.now()
    save_dir = Path(cfg.save_dir) / (str(QUANT_MODES[0]) + '_' + f'{now.strftime("%Y-%m-%d_%H-%M-%S")}')
    ...
    for quant_mode in QUANT_MODES:
        ...
        for video_resolution, video_length in product(VIDEO_RESOLUTIONS, VIDEO_LENGTHS):
            ...
            # Aggressive save
            with open(save_dir / f"latencies__{quant_mode}.json", "w") as f:
                json.dump(results, f)
